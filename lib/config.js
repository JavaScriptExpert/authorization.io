...
// optimized routes
config.views.routes.push({
  path: '/agent',
  template: path.join(__dirname, '../templates/agent.html')
});
config.views.routes.push({
  path: '/register',
  template: path.join(__dirname, '../templates/register.html')
});

bedrock.events.on('bedrock-express.configure.static', function() {
  // serve agent files
  config.express.static.push({
    route: '/css/agent/bedrock-views.css',
    path: path.join(
      config.paths.cache, 'agent', 'bedrock-views', 'app.css'),
    file: true
  });
  config.express.static.push({
    route: '/css/agent/bedrock-views.min.css',
    path: path.join(
      config.paths.cache, 'agent', 'bedrock-views', 'app.min.css'),
    file: true
  });
  config.express.static.push({
    route: '/requirejs/agent/bedrock-angular-templates.js',
    path: path.join(
      config.paths.cache, 'agent', 'bedrock-views', 'angular-templates.js'),
    file: true
  });
  config.express.static.push({
    route: '/requirejs/agent/main.js',
    path: path.join(
      config.paths.cache, 'agent', 'bedrock-requirejs', 'main.js'),
    file: true
  });
  config.express.static.push({
    route: '/requirejs/agent/main.min.js',
    path: path.join(
      config.paths.cache, 'agent', 'bedrock-requirejs', 'main.min.js'),
    file: true
  });
});
